...
if __name__ == '__main__':
    ...
    num_cpu = 1
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # Especifica el dispositivo utilizado
    
    #Variables de entrenamiento

    timesteps = 1000000 # episodios totales de entrenamiento
    ...
    gae_lambda = 0.65 # Expected Rew vs Real Rew
    vf_coef = 0.5
    clip_range = 0.1
    ...
    model.learn(total_timesteps=timesteps, callback=callback, tb_log_name="PPO_LR=0.00003_EnReg=0.1") # Nombre de tensorboard y config del entrenamiento
    model.save(env_id)
    print("------------------Learning Done--------------------")